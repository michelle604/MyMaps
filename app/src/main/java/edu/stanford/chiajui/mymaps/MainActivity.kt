...
import edu.stanford.chiajui.mymaps.MapsAdapter.OnClickListener
...
private const val REQERST_CODE = 1234
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // set layout manager on the recycler review
        rvMaps.layoutManager = LinearLayoutManager(this)
        // set adapter on the recycler view
        mapAdapter = MapsAdapter(this, userMaps, object: OnClickListener {
            override fun onItemClick(position: Int) {
                Log.i(TAG, "onItemClick $position")
                // When user taps on view in RV, we navigate to a new activity
    ...
    private fun showAlertDialog() {
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            ...
            startActivityForResult(intent, REQERST_CODE)
            dialog.dismiss()
        }
    }
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if(requestCode == REQERST_CODE && resultCode == Activity.RESULT_OK) {
